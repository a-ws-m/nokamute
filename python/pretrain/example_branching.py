...
def example_3_train_model():
    ...
    with torch.no_grad():
        ...
        print(
            f"{'Position':<12} {'Rust (raw)':<15} {'Rust (norm)':<15} {'GNN (norm)':<15} {'Diff':<10}"
        )
        print("-" * 70)
        ...
        for i, (node_features, edge_index, rust_eval) in enumerate(test_positions):
            ...
            print(
                f"Position {i+1:<5} {rust_eval:>10.2f}     {rust_norm:>10.4f}     {pred_value:>10.4f}     {diff:>8.4f}"
            )
...
def example_4_compare_parameters():
    ...
    for config in configurations:
        ...
        print(
            f"{config['name']} (p={config['branch_probability']}, max_depth={config['max_depth']}):"
        )
