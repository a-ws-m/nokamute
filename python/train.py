...
from graph_utils import graph_hash, networkx_to_pyg
        ...
        for batch_start in range(0, len(training_data), batch_size):
            ...
            for idx in batch_indices:
                ...
                edge_index_curr = torch.tensor(
                    curr_edge_index, dtype=torch.long, device=device
                )
                ...
                if is_terminal:
                    ...
                    if next_features is not None and len(next_features) > 0:
                        with torch.no_grad():
                            x_next = torch.tensor(
                                next_features, dtype=torch.float32, device=device
                            )
                            edge_index_next = torch.tensor(
                                next_edge_index, dtype=torch.long, device=device
                            )
                            next_batch = torch.zeros(
                                len(x_next), dtype=torch.long, device=device
                            )
            ...
            td_targets = torch.tensor(
                td_targets_list, dtype=torch.float32, device=device
            ).unsqueeze(1)
...
def evaluate_model(model, num_games=50, device="cpu", max_moves=400):
    ...
    player = SelfPlayGame(model=model, epsilon=0.0, device=device, max_moves=max_moves)
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Games per iteration (default: 1 for TD learning)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Number of training epochs (for self-play: epochs per iteration; for pre-training: total epochs)",
    )
    ...
    if args.use_compile:
        if args.device == "cpu":
            print(
                "Note: torch.compile() on CPU may not provide speedup and can cause issues"
            )
            print(
                "      Consider using --no-compile for CPU training, or use GPU with --device cuda"
            )
        else:
            try:
                ...
                print(
                    "✓ Model compiled successfully! Expect 30-200% speedup for inference."
                )
        ...
        if args.pretrain == "eval-matching":
            ...
            if args.pretrain_data_path:
                ...
                pretrain_model_path = os.path.join(
                    args.model_path, "model_eval_matching_latest.pt"
                )
                pretrain_best_model_path = os.path.join(
                    args.model_path, "model_eval_matching_best.pt"
                )
                ...
                def checkpoint_callback(epoch, train_loss, val_loss, is_best):
                    ...
                    if (epoch + 1) % args.save_interval == 0:
                        ...
                        print(
                            f"  Checkpoint saved at epoch {epoch + 1}: {pretrain_model_path}"
                        )
                ...
                if best_model_state is not None:
                    ...
                    print(
                        f"Best pre-trained model saved to: {pretrain_best_model_path}"
                    )
                    print(
                        f"  Best validation loss: {results['best_val_loss']:.6f} at epoch {results['best_epoch'] + 1}"
                    )
                ...
                print(
                    f"  Best validation loss:  {results['best_val_loss']:.6f} (epoch {results['best_epoch'] + 1})"
                )
                ...
                print(
                    f"WARNING: On-the-fly generation doesn't support proper train/val/test split"
                )
                print(
                    f"         Consider using --pretrain-data-path with pre-generated data"
                )
                ...
                total_positions = (
                    args.pretrain_positions * 3
                )  # Generate enough for splitting
                ...
                print(f"Learning rate: {args.lr}")
                print(
                    f"Evaluation depth: {args.pretrain_eval_depth} (0=static, >0=minimax)"
                )
                print(f"Saving checkpoints every {args.save_interval} epochs")
            ...
            print(f"  Test loss:       {results['test_loss']:.6f}")
            print(
                f"  Best val loss:   {results['best_val_loss']:.6f} (epoch {results['best_epoch'] + 1})"
            )
            ...
            for depth, result in pretrain_eval_results.items():
                ...
                if total > 0:
                    ...
                    print(
                        f"  Depth {depth}: {wins}W-{losses}L-{draws}D ({win_rate:.1f}% win rate)"
                    )

            print(f"\nTo continue with self-play training, run:")
            print(
                f"  python train.py --resume {pretrain_best_model_path if best_model_state else pretrain_model_path} --iterations N"
            )
            ...
            pretrain_model_path = os.path.join(
                args.model_path, "model_human_games_latest.pt"
            )
    ...
    if args.replay_buffer:
        buffer_dir = args.replay_buffer_dir or os.path.join(
            args.model_path, "replay_buffer"
        )
        ...
        if stats["total_games"] > 0:
            print(
                f"  Loaded existing buffer: {stats['total_games']:,} games in {stats['num_files']} files"
            )
    ...
    for iteration in range(start_iteration, args.iterations):
        ...
        if args.enable_branching:
            games = player.generate_games_with_branching(
                num_games=args.games, branch_ratio=args.branch_ratio
            )
        else:
        ...
        if args.enable_branching:
            ...
            print(
                f"Collected {num_branches} branch points from {num_nodes} unique positions"
            )
        ...
        if replay_buffer is not None:
            ...
            if total_available > args.replay_sample_size:
                ...
                print(
                    f"Sampled {len(training_games)} games from replay buffer ({total_available} available)"
                )
            else:
        ...
        if replay_buffer is not None:
            buffer_stats = replay_buffer.get_stats()
            writer.add_scalar(
                "ReplayBuffer/total_games", buffer_stats["total_games"], iteration
            )
            writer.add_scalar(
                "ReplayBuffer/num_files", buffer_stats["num_files"], iteration
            )
        ...
        if should_evaluate:
            ...
            if previous_models:
                ...
                for prev_model_name in recent_models:
                    ...
                    prev_checkpoint = os.path.join(
                        args.model_path, f"{prev_model_name}.pt"
                    )
                    ...
                    if os.path.exists(prev_checkpoint):
                        ...
                        prev_model = create_model(prev_checkpoint_data["config"]).to(
                            args.device
                        )
                        prev_model.load_state_dict(
                            prev_checkpoint_data["model_state_dict"]
                        )
                        ...
                        current_player = MLPlayer(
                            model, temperature=0.1, device=args.device
                        )
                        prev_player = MLPlayer(
                            prev_model, temperature=0.1, device=args.device
                        )
                        ...
                        print(
                            f"  vs {prev_model_name}: W={wins} L={losses} D={draws} "
            ...
            checkpoint_path = os.path.join(
                args.model_path, f"model_iter_{iteration}.pt"
            )
            checkpoint_data = {
