...
class EloTracker:
    ...
    def __init__(
        self,
        save_path: str = "checkpoints/elo_history.json",
        k_factor: int = 32,
        ...
        Args:
            ...
            engine_depths: List of engine depths to initialize (if None, initializes all depths 1-5)
        """
        ...
        # Only initialize engine depths that will actually be used
        if engine_depths is None:
            # Default behavior: initialize all depths 1-5
            engine_depths = [1, 2, 3, 4, 5]

        # Initialize ratings for specified engine depths
        # Use a formula: 1000 + depth * 200 (depth 1 = 1200, depth 2 = 1400, etc.)
        for depth in engine_depths:
            self.ratings[f"engine_depth_{depth}"] = 1000 + depth * 200
    ...
    def print_stats(self, player: str):
        ...
        games = [
            ...
            if h["player_a"] == player or h["player_b"] == player
        ]
        ...
        wins = sum(
            1
            for h in games
            if (h["player_a"] == player and h["score_a"] == 1.0)
            or (h["player_b"] == player and h["score_a"] == 0.0)
        )
        draws = sum(1 for h in games if h["score_a"] == 0.5)
